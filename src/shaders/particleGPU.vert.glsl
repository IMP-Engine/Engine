...
uniform sampler2DRect particleSize;
...
void main() {
    ...
    float pSize = texelFetch(particleSize, ivec2(gl_VertexID, 0)).x;
    vec3 pos = texelFetch(position, ivec2(gl_VertexID, 0)).xyz;
    particleSizeOut = pSize;
    gl_Position = modelViewProjectionMatrix * vec4(pos, 1.0);
    gl_PointSize = height * 2*pSize/ gl_Position.w;

    color = colors[1];

    centerPos = (modelViewMatrix * vec4(pos, 1.0)).xyz;
}
